...
//		 cv.put("type", "");
	 ...
	 */
	public static int addAPN(final Context context,APN apn) {
		Log.e(TAG, "in addAPN navtive code");
		if(!checkAPNExists(context, apn.getApn())) {
			deleteAPN(context, apn.getApn());
		}
        ...
        ContentValues cv = new ContentValues();
        cv.put("name", "APN");
        cv.put("apn", apn.getApn());
        cv.put("type", apn.getType());
        cv.put("mcc", apn.getMcc());
        cv.put("mnc", apn.getMnc());
        cv.put("numeric", "");
        cv.put("proxy", "");
        cv.put("port", "80");
        cv.put("current", "1");
        cv.put("mmsproxy", "");
        cv.put("mmsport", "");
        cv.put("user", apn.getUserName());
        cv.put("server", "");
        cv.put("password", apn.getPassword());
        cv.put("mmsc", "");
